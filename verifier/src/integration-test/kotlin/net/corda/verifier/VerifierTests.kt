...
class VerifierTests {
    ...
    fun `single verifier works with a node`() {
        verifierDriver(networkMapStartStrategy = NetworkMapStartStrategy.Dedicated(startAutomatically = true)) {
            ...
            alice.rpc.startFlow(CashIssueFlow::Initiator,
                    10.DOLLARS, OpaqueBytes.of(0), alice.nodeInfo.legalIdentity, notaryFuture.get().nodeInfo.notaryIdentity).returnValue.get()
            notary.waitUntilNumberOfVerifiers(1)
            for (i in 1..10) {
                alice.rpc.startFlow(CashPaymentFlow::Initiator,
                        10.DOLLARS, alice.nodeInfo.legalIdentity).returnValue.get()
            }
        }
    }
}